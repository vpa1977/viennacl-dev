#ifndef VIENNACL_HSA_DEVICE_SPECIFIC_TEMPLATES_TEMPLATE_BASE_
#define VIENNACL_HSA_DEVICE_SPECIFIC_TEMPLATES_TEMPLATE_BASE_
...
/** @file viennacl/hsa_device_specific/templates/template_base.hpp
 *
...
#include "viennacl/hsa_device_specific/lazy_program_compiler.hpp"
#include "viennacl/hsa_device_specific/mapped_objects.hpp"
...
private:
  ...
  public:
    ...
    set_arguments_functor(symbolic_binder & binder, unsigned int & current_arg, vcl::kernel & kernel) : binder_(binder), current_arg_(current_arg), kernel_(kernel){ }
...
protected:
  ...
  void set_arguments(statements_container const & statements, vcl::kernel & kernel, unsigned int & current_arg)
  {
...
public:
  ...
  std::vector<std::string> generate(std::string const & kernel_prefix, statements_container const & statements, vcl::device const & device)
  {
    statements_container::data_type::const_iterator sit;
  ...
  virtual int check_invalid(statements_container const & statements, vcl::device const & device) const = 0;
...
{
private:
  virtual int check_invalid_impl(vcl::device const & /*dev*/) const { return TEMPLATE_VALID; }
...
public:
  ...
  /** @brief returns whether or not the profile has undefined behavior on particular device */
  int check_invalid(statements_container const & statements, vcl::device const & device) const
  {
