...
namespace backend
{
  ...
  *
  */
  inline void memory_create(mem_handle & handle, vcl_size_t size_in_bytes, viennacl::context const & ctx, const void * host_ptr = NULL, bool async = false)
  {
    if (size_in_bytes > 0)
    {
...
#ifdef VIENNACL_WITH_HSA
      case HSA_MEMORY:
    	  handle.hsa_handle() = hsa::memory_create(size_in_bytes,host_ptr);
    	  handle.raw_size(size_in_bytes);
    	break;
#endif
#ifdef VIENNACL_WITH_OPENCL
      case OPENCL_MEMORY:
        ...
        handle.opencl_handle() = opencl::memory_create(handle.opencl_handle().context(), size_in_bytes, host_ptr, async);
        handle.raw_size(size_in_bytes);
        break;
#endif
#ifdef VIENNACL_WITH_CUDA
