...
public:
  static device_specific::execution_handler & execution_handler(viennacl::hsa::context & ctx)
  {
    ...
    if (handlers_map.find(h) == handlers_map.end())
    {
      ...
      handlers_map.insert(std::make_pair(h, ds::execution_handler(viennacl::hsa::type_to_string<NumericT>::apply() + "_vector", ctx, device)));
...
public:
  static device_specific::execution_handler & execution_handler(viennacl::hsa::context & ctx)
  {
    ...
    if (handlers_map.find(h) == handlers_map.end())
    {
      ...
      viennacl::hsa::device const & device = ctx.current_device();
      handlers_map.insert(std::make_pair(h, ds::execution_handler(viennacl::hsa::type_to_string<NumericT>::apply() + "_vector_multi_inner_prod", ctx, device)));
      ds::execution_handler & handler = handlers_map.at(h);
...
public:
  static device_specific::execution_handler & execution_handler(viennacl::hsa::context & ctx)
  {
    ...
    if (handlers_map.find(h) == handlers_map.end())
    {
      ...
      viennacl::hsa::device const & device = ctx.current_device();
      handlers_map.insert(std::make_pair(h, ds::execution_handler(viennacl::hsa::type_to_string<NumericT>::apply() + "_vector_element", ctx, device)));
      ds::execution_handler & handler = handlers_map.at(h);
      ds::vector_axpy_template::parameters_type vector_axpy_params = ds::builtin_database::vector_axpy_params<NumericT>(device);
