#ifndef VIENNACL_LINALG_HSA_VECTOR_OPERATIONS_HPP_
#define VIENNACL_LINALG_HSA_VECTOR_OPERATIONS_HPP_
...
{
  assert(viennacl::traits::hsa_context(dest) == viennacl::traits::hsa_context(src) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::linalg::opencl::kernels::vector_convert< viennacl::hsa::context>::init(ctx);
  viennacl::hsa::kernel& k = ctx.get_kernel(viennacl::linalg::opencl::kernels::vector_convert<>::program_name(), kernel_name);

  viennacl::hsa::enqueue(k( dest, cl_uint(dest.start()), cl_uint(dest.stride()), cl_uint(dest.size()),
...
{
  assert(viennacl::traits::hsa_context(x) == viennacl::traits::hsa_context(y) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute(kernel_name, statement);
}
...
{
  assert(viennacl::traits::hsa_context(x) == viennacl::traits::hsa_context(y) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  assert(viennacl::traits::hsa_context(y) == viennacl::traits::hsa_context(z) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
...
void vector_assign(vector_base<NumericT> & x, const NumericT & alpha, bool up_to_internal_size = false)
{
  ...
  dynamic_cast<device_specific::vector_axpy_template*>(viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).template_of("assign_cpu"))->up_to_internal_size(up_to_internal_size);
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("assign_cpu", statement);
}
...
void vector_swap(vector_base<NumericT> & x, vector_base<NumericT> & y)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("swap", statement);
}
...
{
  assert(viennacl::traits::hsa_context(x) == viennacl::traits::hsa_context(proxy.lhs()) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  assert(viennacl::traits::hsa_context(x) == viennacl::traits::hsa_context(proxy.rhs()) && bool("Vectors do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::linalg::opencl::kernels::vector_element<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute(device_specific::tree_parsing::operator_string(TYPE), statement);
}
                     ...
                     scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("inner_prod", statement);
}

namespace detail
{
  template<typename NumericT>
  viennacl::hsa::packed_cl_uint make_layout(vector_base<NumericT> const & vec)
  {
    viennacl::hsa::packed_cl_uint ret;
                     ...
                     vector_base<NumericT> & result)
{
  ...
  while (current_index < vec_tuple.const_size())
  {
    ...
    viennacl::linalg::opencl::kernels::vector_multi_inner_prod<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute(kernel_prefix, device_specific::statements_container(statements, device_specific::statements_container::INDEPENDENT));
                 ...
                 scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("norm_1", statement);
}
                 ...
                 scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("norm_2", statement);
}
                   ...
                   scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("norm_inf", statement);
}
...
cl_uint index_norm_inf(vector_base<NumericT> const & x)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("index_norm_inf", statement);
                   ...
                   scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("max", statement);
}
                   ...
                   scalar<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("min", statement);
}
...
{
  assert(viennacl::traits::hsa_context(x) == viennacl::traits::hsa_context(y) && bool("Operands do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::linalg::opencl::kernels::vector<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(x)).execute("plane_rotation", statement);
}
...
namespace detail
{
                 ...
                 bool is_inclusive)
  {
    ...
    viennacl::linalg::opencl::kernels::scan<NumericT, viennacl::hsa::context>::init(ctx);
    viennacl::hsa::kernel& k1 = ctx.get_kernel(viennacl::linalg::opencl::kernels::scan<NumericT>::program_name(), "scan_1");
    viennacl::hsa::kernel& k2 = ctx.get_kernel(viennacl::linalg::opencl::kernels::scan<NumericT>::program_name(), "scan_2");
    viennacl::hsa::kernel& k3 = ctx.get_kernel(viennacl::linalg::opencl::kernels::scan<NumericT>::program_name(), "scan_3");

    // First step: Scan within each thread group and write carries
    k1.local_work_size(0, local_worksize);
    k1.global_work_size(0, workgroups * local_worksize);
    viennacl::hsa::enqueue(k1( input, cl_uint( input.start()), cl_uint( input.stride()), cl_uint(input.size()),
                              output, cl_uint(output.start()), cl_uint(output.stride()),
                              cl_uint(is_inclusive ? 0 : 1), opencl_carries.hsa_handle())
                          );

    // Second step: Compute offset for each thread group (exclusive scan for each thread group)
    k2.local_work_size(0, workgroups);
    k2.global_work_size(0, workgroups);
    viennacl::hsa::enqueue(k2(opencl_carries.hsa_handle()));

    // Third step: Offset each thread group accordingly
    k3.local_work_size(0, local_worksize);
    k3.global_work_size(0, workgroups * local_worksize);
    viennacl::hsa::enqueue(k3(output, cl_uint(output.start()), cl_uint(output.stride()), cl_uint(output.size()),
                              opencl_carries.hsa_handle())
                          );
  }
}
