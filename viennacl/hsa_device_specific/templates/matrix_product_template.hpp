#ifndef VIENNACL_HSA_DEVICE_SPECIFIC_TEMPLATES_MATRIX_PRODUCT_HPP
#define VIENNACL_HSA_DEVICE_SPECIFIC_TEMPLATES_MATRIX_PRODUCT_HPP
...
/** @file viennacl/hsa_device_specific/templates/matrix_product_template.hpp
*
...
private:
  ...
  int check_invalid_impl(vcl::device const & /*device*/) const
  {
...
#undef VIENNACL_VSTORE
                     ...
                     std::vector<lazy_program_compiler> & programs, std::string const & kernel_prefix, vcl_size_t id)
  {
    ...
    vcl::kernel& kernel = programs[id].program().get_kernel(kernel_prefix);
  ...
  {
    using namespace hsa_specific::utils;
