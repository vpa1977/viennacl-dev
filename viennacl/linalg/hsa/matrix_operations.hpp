#ifndef VIENNACL_LINALG_HSA_MATRIX_OPERATIONS_HPP_
#define VIENNACL_LINALG_HSA_MATRIX_OPERATIONS_HPP_
...
void convert(matrix_base<DestNumericT> & dest, matrix_base<SrcNumericT> const & src)
{
  ...
  assert(viennacl::traits::hsa_context(dest) == viennacl::traits::hsa_context(src) && bool("Matrices do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::hsa_context(dest));
  viennacl::linalg::opencl::kernels::matrix_convert< viennacl::hsa::context>::init(ctx);
  viennacl::hsa::kernel& k = ctx.get_kernel(viennacl::linalg::opencl::kernels::matrix_convert<>::program_name(), kernel_name);

  viennacl::hsa::enqueue(k( dest, cl_uint(dest.start1()), cl_uint(dest.stride1()), cl_uint(dest.size1()), cl_uint(dest.internal_size1()), cl_uint(dest.start2()), cl_uint(dest.stride2()), cl_uint(dest.size2()), cl_uint(dest.internal_size2()),
        ...
        matrix_base<NumericT> const & B, ScalarT1 const & alpha, vcl_size_t /* len_alpha */, bool reciprocal_alpha, bool flip_sign_alpha)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute(kernel_name, statement);
}
           ...
           matrix_base<NumericT> & temp_trans)
{
  ...
  viennacl::hsa::kernel& kernel = detail::legacy_kernel_for_matrix(proxy.lhs(),kernel_name);
  viennacl::hsa::enqueue(kernel(proxy.lhs(),
...
void matrix_assign(matrix_base<NumericT> & A, NumericT s, bool up_to_internal_size = false)
{
  ...
  dynamic_cast<device_specific::matrix_axpy_template*>(viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).template_of("assign_cpu"))->up_to_internal_size(up_to_internal_size);
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("assign_cpu", statement);
}
...
void matrix_diagonal_assign(matrix_base<NumericT> & A, NumericT s)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("diagonal_assign_cpu", statement);
}
...
void matrix_diag_from_vector(const vector_base<NumericT> & vec, int k, matrix_base<NumericT> & A)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_diag_from_vector", statement);
}
...
void matrix_diag_to_vector(const matrix_base<NumericT> & A, int k, vector_base<NumericT> & vec)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_diag_to_vector", statement);
}
...
void matrix_row(const matrix_base<NumericT> & A, unsigned int i, vector_base<NumericT> & vec)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_row", statement);
}
...
void matrix_column(const matrix_base<NumericT> & A, unsigned int j, vector_base<NumericT> & vec)
{
  ...
  viennacl::linalg::opencl::kernels::matrix<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_column", statement);
}
                ...
                matrix_expression<const matrix_base<NumericT>, const matrix_base<NumericT>, op_element_binary<OpT> > const & proxy)
{
  ...
  assert(viennacl::traits::hsa_context(A) == viennacl::traits::hsa_context(proxy.lhs()) && bool("Matrices do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  assert(viennacl::traits::hsa_context(A) == viennacl::traits::hsa_context(proxy.rhs()) && bool("Matrices do not reside in the same OpenCL context. Automatic migration not yet supported!"));
  ...
  viennacl::linalg::opencl::kernels::matrix_element<NumericT, viennacl::hsa::context>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute(device_specific::tree_parsing::operator_string(TYPE), statement);
}
                     ...
                     vector_base<NumericT> & result)
{
  ...
  viennacl::linalg::opencl::kernels::row_wise_reduction<NumericT, viennacl::hsa::context>::execution_handler(viennacl::traits::hsa_context(A)).execute(kernel_name, statement);
}
               ...
               ScalarType beta)
{
    ...
    viennacl::linalg::opencl::kernels::matrix_prod<NumericT, viennacl::hsa::context>::execution_handler(C.row_major(), viennacl::traits::hsa_context(C)).execute(kernel_prefix, statement);
}
                          ...
                          const vector_base<NumericT> & vec2)
{
  ...
  viennacl::hsa::kernel& kernel= detail::legacy_kernel_for_matrix(A, is_cpu ? "scaled_rank1_update_cpu" : "scaled_rank1_update_gpu");
  ...
  viennacl::hsa::enqueue(kernel(viennacl::traits::hsa_handle(A),
                           ...
                           viennacl::traits::hsa_handle(viennacl::tools::promote_if_host_scalar<NumericT>(alpha)),
                           options_alpha,

                           viennacl::traits::hsa_handle(vec1),
                           cl_uint(viennacl::traits::start(vec1)),
                           cl_uint(viennacl::traits::stride(vec1)),
                           cl_uint(viennacl::traits::size(vec1)),

                           viennacl::traits::hsa_handle(vec2),
                ...
                )
{
  ...
  viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<SCALARTYPE>::program_name(), SVD_BIDIAG_PACK_KERNEL);

  viennacl::hsa::enqueue(kernel(
                                A,
                                D,
                                S,
                ...
                )
{
  ...
  if(A.row_major())
  {
      viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
      viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_BIDIAG_PACK_KERNEL);
  ...
  {
      viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
      viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), SVD_BIDIAG_PACK_KERNEL);
                         ...
                         vcl_size_t start)
{
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_A_LEFT_KERNEL);
        viennacl::hsa::enqueue(kernel(
                                      A,
                                      D,
                                      ...
                                      cl_uint(viennacl::traits::size2(A)),
                                      cl_uint(viennacl::traits::internal_size2(A)),
                                      viennacl::hsa::local_mem(static_cast<cl_uint>(128 * 4))
                              ));
    }
    else
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_A_LEFT_KERNEL);
        viennacl::hsa::enqueue(kernel(
                                      A,
                                      D,
                                      static_cast<cl_uint>(start + 1),
                          ...
                          vector_base<NumericT> & D)
{
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_A_RIGHT_KERNEL);
        ...
        viennacl::hsa::enqueue(kernel(
                                      ...
                                      cl_uint(viennacl::traits::size2(A)),
                                      cl_uint(viennacl::traits::internal_size2(A)),
                                      viennacl::hsa::local_mem(static_cast<cl_uint>(128 * sizeof(NumericT)))
                              ));
    }
    else
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_A_RIGHT_KERNEL);
...
{
    viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::hsa_context(Q));

    if(Q.row_major())
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_QL_KERNEL);
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), SVD_HOUSEHOLDER_UPDATE_QL_KERNEL);
...
template<typename NumericT>
  ...
  {
    viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::hsa_context(matrix));

    if(matrix.row_major())
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_GIVENS_NEXT_KERNEL);
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), SVD_GIVENS_NEXT_KERNEL);
        kernel.global_work_size(0, viennacl::tools::align_to_multiple<cl_uint>(cl_uint(viennacl::traits::size1(matrix)), 256));
        kernel.local_work_size(0, 256);
  ...
  )
  {
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, row_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), kernel_name);
    ...
    {
        viennacl::linalg::opencl::kernels::svd<NumericT, column_major, viennacl::hsa::context>::init(ctx);
        viennacl::hsa::kernel& kernel = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, column_major>::program_name(), kernel_name);
