...
#include "viennacl/linalg/hsa/kernels/svd.hpp"

#include "viennacl/linalg/hsa/kernels/matrix.hpp"
        ...
        matrix_base<NumericT> const & B, ScalarT1 const & alpha, vcl_size_t /* len_alpha */, bool reciprocal_alpha, bool flip_sign_alpha)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute(kernel_name, statement);
}
...
void matrix_assign(matrix_base<NumericT> & A, NumericT s, bool up_to_internal_size = false)
{
  ...
  dynamic_cast<hsa_specific::matrix_axpy_template*>(kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).template_of("assign_cpu"))->up_to_internal_size(up_to_internal_size);
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("assign_cpu", statement);
}
...
void matrix_diagonal_assign(matrix_base<NumericT> & A, NumericT s)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("diagonal_assign_cpu", statement);
}
...
void matrix_diag_from_vector(const vector_base<NumericT> & vec, int k, matrix_base<NumericT> & A)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_diag_from_vector", statement);
}
...
void matrix_diag_to_vector(const matrix_base<NumericT> & A, int k, vector_base<NumericT> & vec)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_diag_to_vector", statement);
}
...
void matrix_row(const matrix_base<NumericT> & A, unsigned int i, vector_base<NumericT> & vec)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_row", statement);
}
...
void matrix_column(const matrix_base<NumericT> & A, unsigned int j, vector_base<NumericT> & vec)
{
  ...
  kernels::matrix<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute("matrix_column", statement);
}
                ...
                matrix_expression<const matrix_base<NumericT>, const matrix_base<NumericT>, op_element_binary<OpT> > const & proxy)
{
  ...
  kernels::matrix_element<NumericT>::execution_handler(A.row_major(), viennacl::traits::hsa_context(A)).execute(hsa_specific::tree_parsing::operator_string(TYPE), statement);
}
                     ...
                     vector_base<NumericT> & result)
{
  ...
  kernels::row_wise_reduction<NumericT>::execution_handler(viennacl::traits::hsa_context(A)).execute(kernel_name, statement);
}
               ...
               ScalarType beta)
{
    ...
    kernels::matrix_prod<NumericT>::execution_handler(C.row_major(), viennacl::traits::hsa_context(C)).execute(kernel_prefix, statement);
}
                          ...
                          const vector_base<NumericT> & vec2)
{
  ...
  viennacl::hsa::enqueue(kernel(viennacl::traits::opencl_handle(A),
                ...
                )
{
  ...
  viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::opencl_handle(A).context());
                         ...
                         vcl_size_t start)
{
    ...
    if(A.row_major())
    {
        ...
        viennacl::hsa::enqueue(kernel(
                                      ...
                                      cl_uint(viennacl::traits::size2(A)),
                                      cl_uint(viennacl::traits::internal_size2(A)),
                                      viennacl::hsa::local_mem(static_cast<cl_uint>(256 * 4))
                              ));
    }
    else
    {
                          ...
                          vector_base<NumericT> & D)
{
    ...
    if(A.row_major())
    {
        ...
        viennacl::hsa::enqueue(kernel(
                                      ...
                                      cl_uint(viennacl::traits::size2(A)),
                                      cl_uint(viennacl::traits::internal_size2(A)),
                                      viennacl::hsa::local_mem(static_cast<cl_uint>(256 * sizeof(NumericT)))
                              ));
    }
    else
    {
...
{
    viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::opencl_handle(Q).context());
...
template<typename NumericT>
  ...
  {
    viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::opencl_handle(matrix).context());
...
#define VIENNACL_SECTION_SIZE 256
                      ...
                      vector_base<NumericT>& vec2)
  {
    ...
    viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::opencl_handle(vec1).context());
    viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::init(ctx);
    viennacl::hsa::kernel& kernel1 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_INCLUSIVE_SCAN_KERNEL_1);
    viennacl::hsa::kernel& kernel2 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_2);
    viennacl::hsa::kernel& kernel3 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_3);
    viennacl::hsa::kernel& kernel4 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_4);

    kernel1.global_work_size(0, VIENNACL_SECTION_SIZE * S.size());
    kernel1.local_work_size(0, VIENNACL_SECTION_SIZE);
    viennacl::hsa::enqueue(kernel1(
                                      viennacl::traits::opencl_handle(vec1),
                      ...
                      vector_base<NumericT>& vec2)
  {
      ...
      viennacl::hsa::context & ctx = const_cast<viennacl::hsa::context &>(viennacl::traits::opencl_handle(vec1).context());
      viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::init(ctx);
      viennacl::hsa::kernel& kernel1 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_EXCLUSIVE_SCAN_KERNEL_1);
      viennacl::hsa::kernel& kernel2 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_2);
      viennacl::hsa::kernel& kernel3 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_3);
      viennacl::hsa::kernel& kernel4 = ctx.get_kernel(viennacl::linalg::opencl::kernels::svd<NumericT, row_major>::program_name(), SVD_SCAN_KERNEL_4);
