#ifndef VIENNACL_LINALG_HSA_DIRECT_SOLVE_HPP
#define VIENNACL_LINALG_HSA_DIRECT_SOLVE_HPP
...
namespace detail
{
  ...
  {
    viennacl::hsa::enqueue(k(viennacl::traits::hsa_handle(A),
                   ...
                   SolverTagT)
{
  ...
  {
    typedef viennacl::linalg::opencl::kernels::matrix_solve<NumericT, row_major, row_major, viennacl::hsa::context>    KernelClass;
    KernelClass::init(ctx);
    program_name = KernelClass::program_name();
  }
  else if (A.row_major() && !B.row_major())
  {
    typedef viennacl::linalg::opencl::kernels::matrix_solve<NumericT, row_major, column_major, viennacl::hsa::context>    KernelClass;
    KernelClass::init(ctx);
    program_name = KernelClass::program_name();
  }
  else if (!A.row_major() && B.row_major())
  {
    typedef viennacl::linalg::opencl::kernels::matrix_solve<NumericT, column_major, row_major, viennacl::hsa::context>    KernelClass;
    KernelClass::init(ctx);
    program_name = KernelClass::program_name();
  }
  else
  {
    typedef viennacl::linalg::opencl::kernels::matrix_solve<NumericT, column_major, column_major, viennacl::hsa::context>    KernelClass;
    KernelClass::init(ctx);
    program_name = KernelClass::program_name();
  }
  ...
  viennacl::hsa::kernel & k = ctx.get_kernel(program_name, ss.str());
                   ...
                   SOLVERTAG)
{
  ...
  viennacl::hsa::kernel & k = detail::legacy_kernel_for_matrix(A,  "triangular_substitute_inplace");
